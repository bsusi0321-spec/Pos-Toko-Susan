...
export async function printBarcodeLabels({ storeName, productName, price, barcode, qty, labelSettings }) {
  ...
  // Sama seperti perbaikan di lib/printReceipt.js: iframe diberi ukuran
  // sungguhan (bukan 0x0) supaya browser tetap menggambar isinya sebelum
  // dicetak, dan menunggu iframe benar-benar selesai dimuat (bukan waktu
  // tunggu tetap) sebelum memanggil cetak -- dua-duanya penyebab umum
  // label/struk "tidak keluar apa-apa" di sebagian HP/browser.
  const iframe = document.createElement("iframe");
  iframe.style.position = "fixed";
  iframe.style.top = "0";
  iframe.style.left = "-99999px";
  iframe.style.width = `${size.w}mm`;
  iframe.style.height = `${size.h * count}mm`;
