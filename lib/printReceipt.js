...
export async function printReceipt({ store, tx, items, cashierName, customerName }) {
  ...
  // PENTING (perbaikan bug "cetak tidak keluar apa-apa"): iframe ini
  // SEBELUMNYA dibuat dengan ukuran 0x0 piksel. Banyak browser (terutama
  // versi Chrome/Android yang lebih baru) tidak repot-repot "menggambar"
  // isi elemen berukuran nol -- karena dianggap tidak akan pernah kelihatan,
  // jadi dilewati untuk hemat kerja. Akibatnya saat print() dipanggil,
  // kadang tidak ada apa pun yang benar-benar sempat digambar untuk
  // dicetak (dialog cetak tidak muncul, atau muncul tapi halaman kosong).
  // Iframe di bawah ini tetap disembunyikan dari pengguna (didorong ke
  // luar layar lewat "left: -9999px"), TAPI ukurannya dibuat sesuai lebar
  // kertas struk sungguhan -- supaya browser tetap menggambar isinya
  // dengan benar sebelum dicetak.
  ...
  // PERBAIKAN KEDUA: sebelumnya menunggu waktu tetap (250 md) sebelum
  // mencetak -- di HP/printer yang lebih lambat, itu kadang belum cukup
  // waktu untuk isi struk selesai digambar, jadi yang tercetak masih
  // kosong/terpotong. Sekarang menunggu iframe-nya BENAR-BENAR selesai
  // dimuat dulu (event "load"), baru mencetak -- lebih bisa diandalkan
  // di perangkat apa pun, cepat atau lambat.
  function triggerPrint() {
    if (iframe.dataset.printed) return; // jaga-jaga supaya tidak tercetak dobel (lihat catatan di bawah)
  ...
  if (doc.readyState === "complete") {
    ...
    // Jaga-jaga kalau event "load" karena suatu hal tidak pernah terpicu
    // di sebagian browser/OS -- tetap coba cetak setelah 1.5 detik. Ditandai
    // lewat "iframe.dataset.printed" di atas supaya tidak sampai tercetak
    // dua kali kalau ternyata "load" tetap terpicu belakangan.
