...
// Dijadikan async supaya bisa menunggu kode QR "ambil struk digital" selesai
// dibuat dulu sebelum dicetak (mengarah ke halaman publik /struk/[id]).
...
export async function printReceipt({ store, tx, items, cashierName, customerName }) {
  ...
  try {
    ...
    if (origin && tx?.id) {
      qrDataUrl = await QRCode.toDataURL(`${origin}/struk/${tx.id}`, { margin: 0, width: 132 });
    }
  } catch {
    // gagal bikin QR bukan alasan buat gagal cetak struknya -- struk tetap
    // dicetak tanpa QR kalau ini gagal.
  }
        ...
        ${
          qrDataUrl
            ? `<div class="center" style="margin-top:6px;">
                 <img src="${qrDataUrl}" width="66" height="66" style="display:inline-block;" />
                 <div class="muted">Scan untuk struk digital</div>
               </div>`
            : ""
        }
      </body>
