...
async function attachDevice(device) {
  ...
  if (!target) {
    ...
    throw new Error(`Printer "${device.productName || "?"}" tersambung tapi tidak ditemukan jalur kirim data cetaknya.`);
  }
  try {
...
// dari klik tombol (tidak bisa dipanggil otomatis di background).
