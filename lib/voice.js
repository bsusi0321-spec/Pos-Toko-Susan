...
import { expandProductNameForVoice } from "@/lib/voiceDictionary";
...
export function speakProductName(name, dictionaryMap) {
  if (typeof window === "undefined") return;
  ...
  try {
    ...
    // "Terjemahkan" singkatan (SCHT, ML, KG, dst) ke kata lengkap dulu lewat
    // Kamus Suara (menu admin) SEBELUM dibacakan -- nama produk aslinya tidak
    // berubah, cuma teks yang dibacakan ini saja. Lihat lib/voiceDictionary.js.
    const textToSpeak = dictionaryMap ? expandProductNameForVoice(name, dictionaryMap) : name;
